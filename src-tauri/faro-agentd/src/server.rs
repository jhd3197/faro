...
/// open). Emits each successful pairing through `on_paired`.
    ...
    F: Fn(String, String) + Send + Sync + Clone + 'static,
{
    loop {
        ...
        tokio::spawn(async move {
            match pair_connection(stream, daemon, &code).await {
                Ok((name, key)) => on_paired(name, key),
...
/// controller's static key, persist. Returns `(client_name, public_key_b64)`.
